...
(() => {
  ...
  const css = `
    ...
    .jury-cart-link{position:fixed;right:16px;bottom:16px;z-index:80;display:none;min-height:48px;align-items:center;justify-content:center;gap:10px;border:1px solid rgba(255,255,255,.16);border-radius:999px;background:var(--accent,#d2ad5a)!important;color:var(--on-accent,#14181c)!important;padding:10px 13px 10px 18px!important;font:700 14px/1 system-ui,sans-serif!important;box-shadow:0 10px 28px rgba(0,0,0,.34)!important}.jury-cart-link.is-visible{display:inline-flex}.jury-cart-link span{display:grid;place-items:center;min-width:26px;height:26px;border-radius:50%;background:var(--on-accent,#14181c);color:var(--ivory-50,#fff);font-size:12px}
    ...
    @media(max-width:767px){.jury-order-layout{grid-template-columns:1fr}.jury-order-checkout{padding:36px 0 54px}.jury-cart-link{right:12px;bottom:calc(76px + env(safe-area-inset-bottom))}.jury-order-modal{padding:10px}.jury-order-modal__head{padding:18px 18px 6px}.jury-order-options{padding:12px 18px 18px}.jury-order-row{grid-template-columns:54px minmax(0,1fr) auto}.jury-order-row__image{width:54px;height:54px}}
  `;
  ...
  const addMenuItem = (menuKey, variantCode = '') => {
    ...
    const selected = variant => add({
      ...
      image: item.image || 'assets/jury-logo.jpg'
    });
  ...
  const mountCheckout = () => {
    ...
    renderCheckout();
    if (location.hash === '#juryOrderCheckout') {
