(() => {
  ...
  Promise.all([menuOverridesReady, fetch('data/menu.json').then((r) => r.json())]).then(([, data]) => {
    ...
    relatedBox.querySelector('div').innerHTML = related.map((item) => { const image = 'assets/jury-logo.jpg'; return `<a href="dish.html?item=${encodeURIComponent(`${cat}:${item.code}`)}"><img src="${image}" alt="${esc(item.name_de)}"><span>${esc(item.name_de)}</span></a>`; }).join('');
